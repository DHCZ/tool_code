...
def project_pts(lidar_pts, l2c_tf, intrinsic, distort, depth_range=(0, 50), height_range=[-0.5, 10],  x_limit=1280, y_limit=720):
    ...
    '''
    cam_pts_3d: the 3d location
    cam_pts_2d: the 2d location in image
    lidar_pts_seg: the seg class of lidar_points
    '''

    lidar_proj_pts = np.concatenate((lidar_pts[:, 0: 3], np.ones((lidar_pts.shape[0], 1))), 1)
    cam_pts_3d = np.dot(l2c_tf, lidar_proj_pts.T)
    ...
    return cam_pts_3d, cam_pts_2d, lidar_pts_seg
