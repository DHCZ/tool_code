  ...
  virtual void Backward(const OpContext &ctx,
    ...
    if (out_data[softmaxout_enum::kOut].shape_ ==
      ...
      if (param_.normalization == softmaxout_enum::kBatch) {
        ...
        using namespace std;
      Tensor<cpu, 3, DType> grad_cpu = NewTensor<cpu>(grad.shape_, DType(0));
        /*Tensor<cpu, 3, DType> grad_cpu =
          ctx.requested[softmaxout_enum::grad].get_host_space_typed<3, DType>(
          grad.shape_);*/
          Copy(grad_cpu, grad, grad.stream_);
        for (index_t i = 0; i < workspace.size(0); ++i) {
          for (index_t j = 0; j < workspace.size(1); ++j) {
           int  gts_label = static_cast<int>(workspace[i][j]);
            DType p_gts = static_cast<int>(1)+grad_cpu[i][gts_label][j];
            if (gts_label == i_label) {
                valid_cnt--;
            }
            /*
            else if((gts_label<0||gts_label>k))
                valid_cnt--;
            else if ((grad_cpu[i][gts_label][j]>-0.1&&grad_cpu[i][gts_label][j]<0))
            {
                //cout<<grad_cpu[i][gts_label][j]<<endl;
                for (index_t c_tmp = 0; c_tmp < k; c_tmp++)
                    grad_cpu[i][c_tmp][j] = DType(0);
                valid_cnt--;
            }  //OHEM CODE*/
            else if((gts_label<0||gts_label>k))
                valid_cnt--;
            else {
            DType temp = pow((1-p_gts), 1)*(2*p_gts*log(p_gts+0.00000000001)+p_gts-1);
            for(index_t c_tmp=0; c_tmp<k; ++c_tmp)
            {
            if (gts_label==c_tmp){
                grad_cpu[i][c_tmp][j] = temp*(1-p_gts);

              }
              else{
                DType p_tmp = grad_cpu[i][c_tmp][j];
                grad_cpu[i][c_tmp][j] = temp*(-p_tmp);
